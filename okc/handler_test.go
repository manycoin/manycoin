...
func testGetNodeData(t *testing.T, protocol int) {
	...
	statedb, _ := okcdb.NewMemDatabase()
...
// Tests that post okc protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", okc63, pm, true)
	defer peer.close()
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, okcash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
