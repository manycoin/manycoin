...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/gokc/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := okcdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = okcdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db okcdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*okcdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db okcdb.Database) {
