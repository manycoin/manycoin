...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Okcoin
...
func (okc *Okcoin) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-okc.shutdownChan:
					return

				case request := <-okc.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := core.GetCanonicalHash(okc.chainDb, (section+1)*params.BloomBitsBlocks-1)
						if compVector, err := core.GetBloomBits(okc.chainDb, task.Bit, section, head); err == nil {
...
// for the Okcoin header bloom filters, permitting blazing fast filtering.
...
func NewBloomIndexer(db okcdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := okcdb.NewTable(db, string(core.BloomBitsIndexPrefix))
