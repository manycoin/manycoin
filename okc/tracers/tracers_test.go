...
import (
	...
	"github.com/okcoin/go-okcoin/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gokc console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = okc.getBlock(okc.getTransaction(tx).blockHash);
  var genesis = okc.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.okc.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  okc.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
