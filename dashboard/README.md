...
The dashboard is a data visualizer integrated into gokc, intended to collect and visualize useful information of an Okcoin node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-okcoin` repo), these need to be installed first:

```
$ (cd dashboard/assets && yarn install && yarn flow)
```

Normally the dashboard assets are bundled into Gokc via `go-bindata` to avoid external dependencies. Rebuilding Gokc after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ gokc --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Gokc, run `go generate`:
