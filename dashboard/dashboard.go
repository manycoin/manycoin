...
func (db *Dashboard) collectData() {
	...
	var (
		...
		prevDiskRead       = metrics.DefaultRegistry.Get("okc/db/chaindata/disk/read").(metrics.Meter).Count()
		prevDiskWrite      = metrics.DefaultRegistry.Get("okc/db/chaindata/disk/write").(metrics.Meter).Count()
	...
	for {
		...
		case <-time.After(db.config.Refresh):
			...
			var (
				...
				curDiskRead       = metrics.DefaultRegistry.Get("okc/db/chaindata/disk/read").(metrics.Meter).Count()
				curDiskWrite      = metrics.DefaultRegistry.Get("okc/db/chaindata/disk/write").(metrics.Meter).Count()
