...
// faucet is a Okcer faucet backed by a light client.
package main
...
var (
	...
	okcPortFlag = flag.Int("okcport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Okcoin protocol")
	statsFlag   = flag.String("okcstats", "", "Okcstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Okcers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Okcoin and the faucet")
)

var (
	okcer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Okcers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *okcPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Okcoin address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Okcoin light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Okcoin protocol stack
	client *okcclient.Client   // Client connection to the Okcoin chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Okcoin light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := okc.DefaultConfig
	...
	// Assemble the okcstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightOkcoin
			ctx.Service(&serv)
			return okcstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := okcclient.NewClient(api)
...
// close terminates the Okcoin connection and tears down the faucet.
...
// apiHandler handles requests for Okcer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, okcer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Okcoin address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somokcing funky happened, please open an issue at https://github.com/okcoin/go-okcoin/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), okcer)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, okcer)
...
// the username, avatar URL and Okcoin address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Okcoin addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Okcoin address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Okcoin address and profile URL.
...
// returning the username, avatar URL and Okcoin address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Okcoin address,
