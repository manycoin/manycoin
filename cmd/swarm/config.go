...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", check github.com/okcoin/go-okcoin/swarm/api/config.go for available fields")
		}
...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (gokc --datadir)
...
func configFileOverride(config *bzzapi.Config, ctx *cli.Context) (*bzzapi.Config, error) {
	...
	//only do somokcing if the -config flag has been set
...
func envVarsOverride(currentConfig *bzzapi.Config) (config *bzzapi.Config) {
	...
	if datadir := os.Getenv(GOKC_ENV_DATADIR); datadir != "" {
		currentConfig.Path = datadir
	}
...
func checkDeprecated(ctx *cli.Context) {
	// exit if the deprecated --okcapi flag is set
	if ctx.GlobalString(DeprecatedOkcAPIFlag.Name) != "" {
		utils.Fatalf("--okcapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
...
func printConfig(config *bzzapi.Config) string {
	...
	if err != nil {
		return fmt.Sprintf("Somokcing is not right with the configuration: %v", err)
	}
	return string(out)
}
