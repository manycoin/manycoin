...
FROM puppokc/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gokc --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gokc --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --okcstats \'{{.Okcstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppokcNetworkID/{{.NetworkID}}/g' dist/js/okcerwallet-master.js && \
	sed -i 's/PuppokcNetwork/{{.Network}}/g'     dist/js/okcerwallet-master.js && \
	sed -i 's/PuppokcDenom/{{.Denom}}/g'         dist/js/okcerwallet-master.js && \
	sed -i 's/PuppokcHost/{{.Host}}/g'           dist/js/okcerwallet-master.js && \
	sed -i 's/PuppokcRPCPort/{{.RPCPort}}/g'     dist/js/okcerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whokcer
// it's running, and if yes, whokcer it's responsive.
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	stats := &walletInfos{
		datadir:  infos.volumes["/root/.okcoin"],
		nodePort: nodePort,
		rpcPort:  rpcPort,
		webHost:  host,
		webPort:  webPort,
