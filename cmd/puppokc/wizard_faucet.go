...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Okcers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.okcstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.okcstats = w.readString() + ":" + w.conf.okcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.okcstats)
		infos.node.okcstats = w.readDefaultString(infos.node.okcstats) + ":" + w.conf.okcstats
	}
