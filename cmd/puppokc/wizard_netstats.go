...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for okcstats availability")
	if infos, err := checkOkcstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["okcstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["okcstats"] = infos.Report()
		okcstats = infos.config
	}
