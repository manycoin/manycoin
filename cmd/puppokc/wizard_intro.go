...
// makeWizard creates and returns a new puppokc wizard.
...
// setting up a new or managing an existing Okcoin private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppokc, your Okcoin private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Okcoin network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and okcstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppokc uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Okcoin network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppokc", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somokcing I can do")
		}
	}
}
