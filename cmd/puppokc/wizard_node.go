...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.okcstats == "" {
		log.Error("No okcstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Okcash != nil && !boot {
		fmt.Println()
		if infos.okcashdir == "" {
			fmt.Printf("Where should the okcash mining DAGs be stored on the remote machine?\n")
			infos.okcashdir = w.readString()
		} else {
			fmt.Printf("Where should the okcash mining DAGs be stored on the remote machine? (default = %s)\n", infos.okcashdir)
			infos.okcashdir = w.readDefaultString(infos.okcashdir)
		}
	}
	...
	if infos.okcstats == "" {
		...
		infos.okcstats = w.readString() + ":" + w.conf.okcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.okcstats)
		infos.okcstats = w.readDefaultString(infos.okcstats) + ":" + w.conf.okcstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Okcash != nil {
			// Okcash based miners only need an okcerbase to mine against
			fmt.Println()
			if infos.okcerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.okcerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.okcerbase)
				infos.okcerbase = w.readDefaultAddress(common.HexToAddress(infos.okcerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Okcoin node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
