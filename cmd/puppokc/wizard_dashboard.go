...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"okcstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "okcstats":
				if infos, err := checkOkcstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have okcstats running, ask whokcer to make the secret public or not
	if w.conf.okcstats != "" {
		fmt.Println()
		fmt.Println("Include okcstats secret on dashboard (y/n)? (default = yes)")
