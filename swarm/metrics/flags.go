...
import (
	...
	gokcmetrics "github.com/okcoin/go-okcoin/metrics"
	"github.com/okcoin/go-okcoin/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if gokcmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gokcmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
