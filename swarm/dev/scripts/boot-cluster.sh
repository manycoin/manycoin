...
#
# The cluster contains a bootnode, a gokc node and multiple swarm nodes, with
...
# static gokc configuration
GOKC_IP="192.168.33.3"
GOKC_RPC_PORT="8545"
GOKC_RPC_URL="http://${GOKC_IP}:${GOKC_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gokc and
# swarm nodes
start_bootnode() {
...
# start_gokc_node starts a gokc node with --datadir pointing at <base-dir>/gokc
# and a single, unlocked account with password "gokc"
start_gokc_node() {
  local dir="${base_dir}/gokc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gokc account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GOKC_IP}"
    --rpcport   "${GOKC_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gokc" "${GOKC_IP}" "$(which gokc)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vokc pair
  local vokc0="vokc${name}0"
  local vokc1="vokc${name}1"
  ip link add name "${vokc0}" type vokc peer name "${vokc1}"

  # add one end to the bridge
  ip link set dev "${vokc0}" master "${BRIDGE_NAME}"
  ip link set dev "${vokc0}" up

  # add the other end to the namespace, rename it okc0 and give it the ip
  ip link set dev "${vokc1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vokc1}" name "okc0"
  ip netns exec "${name}" ip link set dev "okc0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "okc0"
}
...
create_account() {
  ...
  gokc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
