...
import (
	...
	"github.com/okcoin/go-okcoin/swarm/storage"
)
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.okc"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.okc"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.okc"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	okcAddr := "swarm.okc"
	okcHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	okcResolve := newTestResolver(okcHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(okcResolve, "")),
			addr:   okcAddr,
			result: okcHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(okcResolve, ""),
				MultiResolverOptionWithResolver(okcResolve, ""),
			),
			addr:   okcAddr,
			result: okcHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(okcResolve, "okc"),
			),
			addr:   okcAddr,
			result: okcHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "okc"),
				MultiResolverOptionWithResolver(doesntResolve, "okc"),
				MultiResolverOptionWithResolver(okcResolve, "okc"),
			),
			addr:   okcAddr,
		...
		},
		{
			...
			addr:   okcAddr,
			result: okcHash,
			err:    errors.New(`DNS name not found: "swarm.okc"`),
		},
		{
