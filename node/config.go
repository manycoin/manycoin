...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gokc is "gokc". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gokc", keep that.
	if name == "gokc" || name == "gokc-testnet" {
		name = "Gokc"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gokc" instances.
var isOldGokcResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by gokc 1.4 are used if they exist.
	if c.name() == "gokc" && isOldGokcResource[path] {
		oldpath := ""
		if c.Name == "gokc" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-okcoin-keystore")
		ephemeral = keydir
	}
