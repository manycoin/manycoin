...
func (n *Node) OpenDatabase(name string, cache, handles int) (okcdb.Database, error) {
	if n.config.DataDir == "" {
		return okcdb.NewMemDatabase()
	}
	return okcdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
