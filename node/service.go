...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (okcdb.Database, error) {
	if ctx.config.DataDir == "" {
		return okcdb.NewMemDatabase()
	}
	db, err := okcdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
