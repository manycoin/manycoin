...
type request struct {
	...
	raw  bool        // Whokcer this is a raw entry (code) or a trie node
...
// Process injects a batch of retrieved trie nodes data, returning if somokcing
...
func (s *TrieSync) Commit(dbw okcdb.Putter) (int, error) {
...
func (s *TrieSync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whokcer known or not
	type child struct {
		node  node
		depth int
	}
