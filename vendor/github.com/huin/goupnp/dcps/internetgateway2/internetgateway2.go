...
const (
	...
	URN_WANOkcernetLinkConfig_1    = "urn:schemas-upnp-org:service:WANOkcernetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Okcernet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Okcernet
//
...
// WANOkcernetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANOkcernetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANOkcernetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANOkcernetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANOkcernetLinkConfig1Clients() (clients []*WANOkcernetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANOkcernetLinkConfig_1); err != nil {
		return
	}
	clients = newWANOkcernetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANOkcernetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANOkcernetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANOkcernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANOkcernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANOkcernetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANOkcernetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANOkcernetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANOkcernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANOkcernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANOkcernetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANOkcernetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANOkcernetLinkConfig1 {
	clients := make([]*WANOkcernetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANOkcernetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewOkcernetLinkStatus: allowed values: Up, Down
func (client *WANOkcernetLinkConfig1) GetOkcernetLinkStatus() (NewOkcernetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewOkcernetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANOkcernetLinkConfig_1, "GetOkcernetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewOkcernetLinkStatus, err = soap.UnmarshalString(response.NewOkcernetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
