...
// Reference: https://github.com/okcoin/wiki/wiki/Okcash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see okcash_io_prepare
enum okcash_io_rc {
	OKCASH_IO_FAIL = 0,           ///< There has been an IO failure
	OKCASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	OKCASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	OKCASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of okcash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * okcash_full_t
 */
#ifdef OKCASH_PRINT_CRITICAL_OUTPUT
#define OKCASH_CRITICAL(...)							\
	do													\
	{													\
		printf("OKCASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for okcash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the okcash
 ...
 *                           https://github.com/okcoin/wiki/wiki/Okcash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum okcash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/okcoin/wiki/wiki/Okcash-DAG
 *
...
static inline bool okcash_io_mutable_name(
	uint32_t revision,
