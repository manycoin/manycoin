...
var (
	...
	configPrefix   = []byte("okcoin-config-") // config prefix for the db
...
func WriteCanonicalHash(db okcdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db okcdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db okcdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db okcdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db okcdb.Putter, count uint64) error {
...
func WriteHeader(db okcdb.Putter, header *types.Header) error {
...
func WriteBody(db okcdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db okcdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db okcdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db okcdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db okcdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db okcdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db okcdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db okcdb.Database) okcdb.Database {
	return okcdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db okcdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db okcdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db okcdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
