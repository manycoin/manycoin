...
// Package core implements the Okcoin consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whokcer to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     okcdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Okcoin Validator and
// Processor.
func NewBlockChain(db okcdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= okcdb.IdealBatchSize {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nonokceless.
...
//
// The verify parameter can be used to fine tune whokcer nonce verification
