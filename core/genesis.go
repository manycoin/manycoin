...
func SetupGenesisBlock(db okcdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllOkcashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
	...
	// Check whokcer the genesis block is already written.
	if genesis != nil {
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	default:
		return params.AllOkcashProtocolChanges
	}
}

// ToBlock creates the genesis block and writes state of a genesis specification
// to the given database (or discards it if nil).
func (g *Genesis) ToBlock(db okcdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db okcdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
	...
	config := g.Config
	if config == nil {
		config = params.AllOkcashProtocolChanges
	}
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db okcdb.Database) *types.Block {
...
func GenesisBlockForTesting(db okcdb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the Okcoin main net genesis block.
...
// DeveloperGenesisBlock returns the 'gokc --dev' genesis block. Note, this must
// be seeded with the
