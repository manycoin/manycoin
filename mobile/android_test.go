...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gokc.LightScryptN, Gokc.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.okcoin", new NodeConfig());
			node.start();
			...
			OkcoinClient ec = node.getOkcoinClient();
	...
	// signatures too. Regression test for go-okcoin issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gokc-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/okcoin/gokctest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gokc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.okcoin", "github.com/okcoin/go-okcoin/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gokc.aar"), "gokc.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "okcoin", "gokctest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
