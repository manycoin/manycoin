...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Okcoin
...
type wallet struct {
	...
	deriveChain    okcoin.ChainStateReader // Blockchain state reader to discover used account with
...
// whokcer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Okcoin account
			...
			// If the next account is empty, stop self-derivation, but add it nonokceless
...
// Contains implements accounts.Wallet, returning whokcer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain okcoin.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
