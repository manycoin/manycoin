...
import (
	...
	"github.com/okcoin/go-okcoin/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whokcer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whokcer the Trezor is opened, closed
// or whokcer the Okcoin app was not started on it.
...
// Okcoin address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.OkcoinAddress)
	if _, err := w.trezorExchange(&trezor.OkcoinGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.OkcoinTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.OkcoinTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
