...
func NewLesServer(okc *okc.Okcoin, config *okc.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(okc.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, okc.EventMux(), okc.Engine(), newPeerSet(), okc.BlockChain(), okc.TxPool(), okc.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(okc.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(okc.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(okc.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(okc.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(okc.ChainDb())
	return srv, nil
}
...
func newCostStats(db okcdb.Database) *requestCostStats {
