...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb, _  = okcdb.NewMemDatabase()
		lightdb, _ = okcdb.NewMemDatabase()
		gspec      = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis    = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, okcash.NewFullFaker(), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, okcash.NewFaker(), fulldb, 4, testChainGen)
