...
// Package miner implements Okcoin block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whokcer we can start the mining operation
	shouldStart int32 // should start indicates whokcer we should start after sync
}

func New(okc Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		okc:      okc,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, okc, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(okc.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setOkcerbase(coinbase)
...
func (self *Miner) SetOkcerbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setOkcerbase(addr)
}
