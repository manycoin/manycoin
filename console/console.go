...
import (
	...
	"github.com/okcoin/go-okcoin/internal/jsre"
	"github.com/okcoin/go-okcoin/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Okcoin requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jokc", struct{}{})

	jokcObj, _ := c.jsre.Get("jokc")
	jokcObj.Object().Set("send", bridge.Send)
	jokcObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jokc);"); err != nil {
	...
	flatten := "var okc = web3.okc; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = okc.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jokc.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jokc.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jokc.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jokc.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jokc.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines okc.getBalance(okc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gokc instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gokc metadata
	fmt.Fprintf(c.printer, "Welcome to the Gokc JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + okc.coinbase);
		console.log("at block: " + okc.blockNumber + " (" + new Date(1000 * okc.getBlock(okc.blockNumber).timestamp) + ")");
