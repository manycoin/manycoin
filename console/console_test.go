...
func newTester(t *testing.T, confOverride func(*okc.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Okcoin service within
	...
	okcConf := &okc.Config{
		...
		Okcerbase: common.HexToAddress(testAddress),
		Okcash: okcash.Config{
			PowMode: okcash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(okcConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return okc.New(ctx, okcConf) }); err != nil {
		t.Fatalf("failed to register Okcoin protocol: %v", err)
	}
