...
type Vote struct {
	...
	Authorize bool           `json:"authorize"` // Whokcer to authorize or deauthorize the voted account
}

// Tally is a simple vote tally to keep the current score of votes. Votes that
// go against the proposal aren't counted since it's equivalent to not voting.
type Tally struct {
	Authorize bool `json:"authorize"` // Whokcer the vote is about authorizing or kicking someone
...
func loadSnapshot(config *params.CliqueConfig, sigcache *lru.ARCCache, db okcdb.Database, hash common.Hash) (*Snapshot, error) {
...
func (s *Snapshot) store(db okcdb.Database) error {
...
// validVote returns whokcer it makes sense to cast the specified vote in the
