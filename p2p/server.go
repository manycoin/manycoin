...
// Package p2p implements the Okcoin p2p network protocols.
package p2p
...
type Config struct {
	...
	// DiscoveryV5 specifies whokcer the the new topic-discovery based V5 discovery
	...
	if !srv.NoDiscovery || srv.DiscoveryV5 {
		...
		if srv.NAT != nil {
			if !realaddr.IP.IsLoopback() {
				go nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "okcoin discovery")
			}
...
func (srv *Server) startListening() error {
	...
	if !laddr.IP.IsLoopback() && srv.NAT != nil {
		srv.loopWG.Add(1)
		go func() {
			nat.Map(srv.NAT, srv.quit, "tcp", laddr.Port, laddr.Port, "okcoin p2p")
			srv.loopWG.Done()
		}()
	}
	return nil
